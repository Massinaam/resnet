...
def prune_structured_model(model, pruning_ratio=0.2):
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d):
            # Pruning structuré: suppression de certains filtres
            prune.ln_structured(module, name='weight', amount=pruning_ratio, dim=0, n=1)  # 'n' spécifie le nombre de filtres à conserver
            prune.remove(module, 'weight')  # Applique définitivement le pruning

# Appliquer le pruning structuré au modèle
#prune_structured_model(model, pruning_ratio=0.2)
